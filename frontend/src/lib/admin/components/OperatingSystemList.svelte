<script lang="ts">
	...
	function iconFor(name: string) {
		switch (name) {
			case 'windows':
				return { icon: 'logos:microsoft-windows-icon', color: '#0078D4' };
			case 'macos':
				return { icon: 'logos:apple', color: '#000000' };
			case 'linux':
				return { icon: 'logos:linux-tux', color: '#FCC624' };
			case 'android':
				return { icon: 'logos:android-icon', color: '#3DDC84' };
			case 'ios':
				return { icon: 'logos:apple', color: '#000000' };
			case 'chromeos':
				return { icon: 'logos:chrome', color: '#4285F4' };
			case 'ubuntu':
				return { icon: 'logos:ubuntu', color: '#E95420' };
			case 'debian':
				return { icon: 'logos:debian', color: '#A81D33' };
			case 'fedora':
				return { icon: 'logos:fedora', color: '#294172' };
			case 'arch':
				return { icon: 'logos:archlinux', color: '#1793D1' };
			default:
				return { icon: 'lucide:monitor', color: 'var(--accent-color, #6366f1)' };
		}
	}
