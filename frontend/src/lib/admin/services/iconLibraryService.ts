...
async function fetchLucideIcons(): Promise<IconOption[]> {
	try {
		...
		Object.keys(lucideModule).forEach((name) => {
			if (name !== 'default' && name !== 'createLucideIcon' && !commonIcons.includes(name)) {
				const component = (lucideModule as any)[name];
