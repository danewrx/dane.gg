...
export class TwitterScheduler {
  ...
  static async initialize(): Promise<void> {
    ...
    try {
      ...
      if (!username) {
        console.log('[Twitter Scheduler] Twitter username not configured. Set it in the admin panel or TWITTER_USERNAME environment variable');
        return;
      }
      ...
      if (process.env.TWITTER_FETCH_CRON) {
        ...
        const pollIntervalMs = Math.max(
          parseInt(process.env.TWITTER_POLL_INTERVAL, 10),
          30000
        );
        const pollIntervalMinutes = Math.floor(pollIntervalMs / 60000);
        fetchCronExpression = `*/${pollIntervalMinutes} * * * *`;
        console.log(`[Twitter Scheduler] ⚠️  TWITTER_POLL_INTERVAL is deprecated. Use TWITTER_FETCH_CRON instead.`);
      } else {
      ...
      this.fetchJob = cron.schedule(fetchCronExpression, async () => {
        try {
          // Get username dynamically
          const currentDbUsername = await ConfigService.get('twitter_username');
          const currentUsername = (currentDbUsername && typeof currentDbUsername === 'string' && currentDbUsername.trim())
      ...
      this.healthCheckJob = cron.schedule(healthCheckCronExpression, async () => {
        try {
          // Get username dynamically
          const currentDbUsername = await ConfigService.get('twitter_username');
          const currentUsername = (currentDbUsername && typeof currentDbUsername === 'string' && currentDbUsername.trim())
      ...
      console.log(`[Twitter Scheduler] ⚠️  WARNING: Polling too frequently may trigger Twitter rate limits or account restrictions`);
    } catch (error: any) {
