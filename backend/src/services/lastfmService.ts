...
export class LastFmService {
	...
	static async getRecentTracks(limit: number = 1): Promise<LastFmTrack[]> {
		...
		try {
			...
			return tracks.map((track: LastFmTrackRaw): LastFmTrack => ({
				artist: typeof track.artist === 'string' ? track.artist : track.artist['#text'],
				name: track.name,
				album: track.album ? (typeof track.album === 'string' ? track.album : track.album['#text']) : undefined,
				image: this.getBestImage(track.image),
				url: track.url ? (typeof track.url === 'string' ? track.url : track.url['#text']) : undefined,
	...
	static async getCurrentMusicStatus(): Promise<{
		...
		try {
			...
			if (lastPlayed) {
				...
				console.log(`Found last played track: ${lastPlayed.artist} - ${lastPlayed.name} (played at: ${lastUpdate})`);
				return {
