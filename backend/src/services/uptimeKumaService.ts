...
export class UptimeKumaService {
  ...
  static async getAllMonitors(): Promise<UptimeKumaMonitor[]> {
    ...
    if (this.apiKey) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(`[Uptime Kuma] REST API response type:`, Array.isArray(data) ? 'array' : typeof data);
          console.log(`[Uptime Kuma] REST API response keys:`, Array.isArray(data) ? 'array' : Object.keys(data));
          ...
          errors.push(`REST API endpoint failed (${response.status}): ${errorText.substring(0, 100)}`);
          console.log(`[Uptime Kuma] REST API endpoint failed: ${response.status} ${response.statusText}`);
        }
      } catch (error: any) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`[Uptime Kuma] Metrics endpoint failed: ${response.status} ${response.statusText}`);
      }
    } catch (error: any) {
    ...
    for (const slug of statusPageSlugs) {
      try {
        console.log(`[Uptime Kuma] Attempting status page at ${this.baseUrl}/api/status-page/${slug}`);
    ...
    try {
      console.log(`[Uptime Kuma] Attempting heartbeat endpoint at ${this.baseUrl}/api/status-page/heartbeat`);
    ...
    if (monitors.length === 0) {
      console.log('[Uptime Kuma] All API attempts failed, attempting to load from database cache...');
      try {
  ...
  static async getMonitorStatus(monitorIds: number[]): Promise<UptimeKumaMonitor[]> {
    ...
    if (this.apiKey) {
      try {
        console.log(`[Uptime Kuma] Attempting REST API endpoint for ${monitorIds.length} selected monitors`);
        const authHeader = `Basic ${Buffer.from(`api:${this.apiKey}`).toString('base64')}`;
        const response = await fetch(`${this.baseUrl}/api/monitor`, {
          method: 'GET',
          headers: {
    ...
    try {
      ...
      if (monitors.length > 0) {
        console.log(`[Uptime Kuma] Loaded ${monitors.length} selected monitors from database cache`);
        return monitors;
      }
    } catch (cacheError: any) {
      console.error('[Uptime Kuma] Failed to load from cache:', cacheError.message);
        ...
        await db
          ...
          .onConflictDoUpdate({
            ...
            set: {
              ...
              customName: monitor.customName !== undefined ? monitor.customName : existingCustomName,
              url: monitor.url,
              type: monitor.type,
              status: monitor.status,
              group: monitor.group,
  ...
  private static async mergeCustomNames(monitors: UptimeKumaMonitor[]): Promise<void> {
    try {
      ...
      const cached = await db
        .select({ monitorId: uptimeKumaMonitors.monitorId, customName: uptimeKumaMonitors.customName })
        .from(uptimeKumaMonitors)
  ...
  static async updateCustomNames(customNames: Record<number, string | null>): Promise<void> {
    try {
      ...
      console.log(`[Uptime Kuma] Updated custom names for ${Object.keys(customNames).length} monitors`);
    } catch (error: any) {
  ...
  private static parseRestApiMonitors(data: any[]): UptimeKumaMonitor[] {
    ...
    for (const item of data) {
      ...
      const groupName = item.parentId && groups.has(item.parentId) ? groups.get(item.parentId) : undefined;
    ...
    console.log(`[Uptime Kuma] Parsed ${monitors.length} monitors from REST API, ${groupedCount} with groups`);
    
    if (groupedCount === 0 && monitors.length > 0) {
      console.log(`[Uptime Kuma] No groups assigned from REST API, falling back to type-based grouping`);
      ...
      console.log(`[Uptime Kuma] ${monitors.length - groupedCount} monitors without groups, assigning type-based groups`);
      const typeMap: Record<string, string> = {
        'http': 'Websites',
        'https': 'Websites',
        'keyword': 'Websites', // HTTP(s) - Keyword
  ...
  private static parseMonitors(data: any[]): UptimeKumaMonitor[] {
    return data.map((item: any) => {
      const monitor: UptimeKumaMonitor = {
        ...
        avgResponseTime: item.avgResponseTime || item.avg_response_time || item.monitor?.avgResponseTime,
  ...
  private static parseStatus(status: any): 'up' | 'down' | 'pending' | 'maintenance' {
    ...
    if (statusStr === 'up' || statusStr === 'online' || statusStr === '1' || statusStr === 'true') {
      return 'up';
    } else if (statusStr === 'down' || statusStr === 'offline' || statusStr === '0' || statusStr === 'false') {
      return 'down';
