...
router.get('/', async (req, res) => {
	try {
		const certs = await db.select().from(certifications).orderBy(asc(certifications.displayOrder));
...
router.post('/', requireAuth, async (req, res) => {
	try {
		...
		const maxOrder = allCerts.reduce((max, cert) => Math.max(max, cert.displayOrder), -1);
