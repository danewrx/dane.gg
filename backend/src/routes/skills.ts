...
router.post('/skills', requireAuth, async (req, res) => {
	try {
		...
		const categorySkills = await db.select().from(skills).where(eq(skills.categoryId, categoryId));

		const maxOrder = categorySkills.reduce((max, skill) => Math.max(max, skill.displayOrder), -1);
