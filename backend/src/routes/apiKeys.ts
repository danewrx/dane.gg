...
router.post('/', requireSession, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (error.code === '23505') {
      return res.status(409).json({ error: 'An API key with this prefix already exists. Please try again.' });
    }
 ...
 */
router.post('/:id/regenerate', requireSession, requireAdmin, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
