...
/* Post admin upload */
router.post('/', requireSession, requireAdmin, soundUpload.single('file'), async (req: Request, res: Response) => {
  try {
    if (!req.file) {
    ...
    if (displayName.length > 120) {
      if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
      return res.status(400).json({ success: false, error: 'Display name must be 120 characters or less' });
    }
...
router.delete('/:id', requireSession, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (row.id === DEFAULT_CHAT_NOTIFICATION_SOUND_ID || row.name === DEFAULT_CHAT_NOTIFICATION_SOUND_NAME) {
      return res.status(403).json({ success: false, error: 'The built-in default notification sound cannot be deleted' });
    }
