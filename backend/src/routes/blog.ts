...
router.post('/admin', requireSession, async (req, res) => {
	try {
		...
		const [existingPost] = await db.select().from(posts).where(eq(posts.slug, finalSlug)).limit(1);
		...
		if (tagNames && Array.isArray(tagNames) && tagNames.length > 0) {
			for (const tagName of tagNames) {
				let [tag] = await db.select().from(blogTags).where(eq(blogTags.name, tagName)).limit(1);

				if (!tag) {
					[tag] = await db.insert(blogTags).values({ name: tagName }).returning();
				}
...
router.put('/admin/:id', requireSession, async (req, res) => {
	try {
		...
		if (slug && slug !== existingPost.slug) {
			const [conflictingPost] = await db.select().from(posts).where(eq(posts.slug, slug)).limit(1);
...
router.get('/admin/tags/all', requireSession, async (req, res) => {
	try {
		const allTags = await db.select().from(blogTags).orderBy(blogTags.name);
...
router.post('/admin/tags', requireSession, async (req, res) => {
	try {
		...
		const [newTag] = await db.insert(blogTags).values({ name: name.trim() }).returning();
...
router.put('/admin/tags/:id', requireSession, async (req, res) => {
	try {
		...
		const [existingTag] = await db.select().from(blogTags).where(eq(blogTags.id, id)).limit(1);
...
router.delete('/admin/tags/:id', requireSession, async (req, res) => {
	try {
		...
		const [deletedTag] = await db.delete(blogTags).where(eq(blogTags.id, id)).returning();
...
router.get('/:slug/navigation', async (req, res) => {
	try {
		...
		const [previousPost] = await db
			...
			.from(posts)
			.where(and(eq(posts.published, true), lt(posts.publishedAt, currentPost.publishedAt)))
			.orderBy(desc(posts.publishedAt))
			.limit(1);
		...
		const [nextPost] = await db
			...
			.from(posts)
			.where(and(eq(posts.published, true), gt(posts.publishedAt, currentPost.publishedAt)))
