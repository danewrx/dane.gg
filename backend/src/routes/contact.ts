...
router.post('/emails', requireAuth, async (req, res) => {
  try {
    ...
    const maxOrder = allEmails.reduce((max, e) => 
      Math.max(max, e.displayOrder), -1);

    const [newEmail] = await db.insert(contactEmails).values({
      description,
      email,
...
router.put('/emails/:id', requireAuth, async (req, res) => {
  try {
    ...
    const [updatedEmail] = await db.update(contactEmails)
      .set(updateData)
...
router.delete('/emails/:id', requireAuth, async (req, res) => {
  try {
    ...
    const [deletedEmail] = await db.delete(contactEmails)
      .where(eq(contactEmails.id, id))
      .returning();
...
router.put('/settings/:key', requireAuth, async (req, res) => {
  try {
    ...
    let result;
    if (existing) {
      [result] = await db.update(contactPageSettings)
        .set({ 
          value,
          updatedAt: new Date()
        })
      ...
      [result] = await db.insert(contactPageSettings)
        .values({
          key,
          value
        })
...
router.delete('/settings/:key', requireAuth, async (req, res) => {
  try {
    ...
    const [deleted] = await db.delete(contactPageSettings)
      .where(eq(contactPageSettings.key, key))
      .returning();
