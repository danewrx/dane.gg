 ...
 */
router.post('/', requireSession, defaultUpload.single('file'), async (req: Request, res: Response) => {
  try {
    if (!req.file) {
    ...
    // Save to database
    const [uploadRecord] = await db.insert(userUploads).values({
      userId: req.user.id,
 ...
 */
router.post('/custom', requireSession, (req: Request, res: Response, next) => {
  ...
  const allowedTypes = allowedTypesParam 
    ...
    : ['image'];
  const maxSize = maxSizeParam 
    ? parseInt(maxSizeParam, 10)
    : 10 * 1024 * 1024;
