...
async function migrateContactData() {
  ...
  try {
    ...
    const emailsConfig = await db.select()
      .from(siteConfig)
    ...
    const emailsHeaderConfig = await db.select()
      .from(siteConfig)
    ...
    const socialLinksConfig = await db.select()
      .from(siteConfig)
    ...
    const socialHeaderConfig = await db.select()
      .from(siteConfig)
    ...
    const taglineConfig = await db.select()
      .from(siteConfig)
    ...
    console.log('   3. Once confirmed working, you can optionally delete the old keys from site_config:\n');
...
async function migrateSettingToContactPage(key: string, value: string) {
  ...
  if (existing.length === 0) {
    ...
    await db.update(contactPageSettings)
