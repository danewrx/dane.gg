<script lang="ts">
	...
	let categories = $state<ThemeCategory[]>([]);
	let loading = $state(true);
	let isSaving = $state(false);
	let showAddForm = $state(false);
	let editingCategory: ThemeCategory | null = $state(null);
	...
	async function loadCategories() {
		try {
			loading = true;
			categories = await getAllThemeCategories();
			...
			if (allHaveZeroOrder && categories.length > 0) {
				...
				try {
					await updateThemeCategoryOrder(categoryOrders);
					categories = await getAllThemeCategories();
				} catch (err) {
	...
	function startEdit(category: ThemeCategory) {
	...
	async function saveCategory() {
		...
		try {
			isSaving = true;
			if (editingCategory) {
				await updateThemeCategory(editingCategory.id, formData.name.trim());
				toast.success('Category updated', {
					description: `"${formData.name.trim()}" has been updated`
				});
			} else {
				await createThemeCategory(formData.name.trim());
				...
				<div class="modal-content">
					<div class="form-group">
						...
						<input
							...
							placeholder="e.g., Retro, Editor, Special"
							required
							class="form-input"
							onkeydown={(e) => {
