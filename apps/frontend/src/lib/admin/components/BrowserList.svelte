<script lang="ts">
	...
	function iconFor(name: string) {
		switch (name) {
			case 'chrome':
				return { icon: 'logos:chrome', color: '#4285F4' };
			case 'chromium':
				return { icon: 'logos:chromium', color: '#4285F4' };
			case 'firefox':
				return { icon: 'logos:firefox', color: '#FF7139' };
			case 'safari':
				return { icon: 'logos:safari', color: '#0FB5EE' };
			case 'edge':
				return { icon: 'logos:microsoft-edge', color: '#0A7CF2' };
			case 'opera':
				return { icon: 'logos:opera', color: '#FF1B2D' };
			case 'brave':
				return { icon: 'logos:brave', color: '#FB542B' };
			case 'samsung-internet':
				return { icon: 'logos:samsung-internet', color: '#1428A0' };
			case 'vivaldi':
				return { icon: 'logos:vivaldi-icon', color: '#EF3939' };
			case 'uc':
				return { icon: 'logos:uc-browser', color: '#E36F00' };
			case 'ie':
				return { icon: 'logos:ie', color: '#1EBBEE' };
			default:
				return { icon: 'lucide:globe', color: 'var(--accent-color, #6366f1)' };
		}
	}
	...
	const total = $derived(
		data.reduce(
			(s, d) =>
				s + (typeof d.visitors === 'number' ? d.visitors : parseInt(d.visitors as any) || 0),
			0
		)
	);
	...
	const items = $derived(
		data.map((d) => {
			...
			const visitors =
				typeof d.visitors === 'number' ? d.visitors : parseInt(d.visitors as any) || 0;
